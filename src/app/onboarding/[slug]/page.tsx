// Per-plan onboarding field lists (for building the real Tally forms later)
// live in /docs/onboarding-field-lists.md — not rendered on-site.
import type { Metadata } from "next";
import { notFound } from "next/navigation";
import { ClipboardCheck } from "lucide-react";
import { Breadcrumbs } from "@/components/ui/Breadcrumbs";
import { onboardingEntries, getOnboardingEntry } from "@/config/onboarding";
...
export function generateStaticParams() {
  return onboardingEntries.map((entry) => ({ slug: entry.slug }));
}

export async function generateMetadata({ params }: OnboardingPageProps): Promise<Metadata> {
  const { slug } = await params;
  const entry = getOnboardingEntry(slug);

  if (!entry) {
    return buildMetadata({
      title: "Not Found | AeroScale",
      description: "This onboarding page could not be found.",
  ...
  return buildMetadata({
    title: `Get Started with ${entry.name} | AeroScale`,
    description: `Finish setting up ${entry.name} with AeroScale.`,
    path: `/onboarding/${entry.slug}`,
  });
}
...
export default async function OnboardingPage({ params }: OnboardingPageProps) {
  ...
  const embedIsPlaceholder = isPlaceholderLink(entry.onboardingFormEmbed);

  return (
    <>
      <Breadcrumbs
        items={[{ name: "Home", path: "/" }, { name: `${entry.name} Onboarding`, path: `/onboarding/${entry.slug}` }]}
      />
      ...
      <section className="bg-white py-16 md:py-24">
        <div className="container-page max-w-2xl">
          ...
          <h1 className="mt-4 text-[clamp(2rem,3.6vw,3rem)] leading-[1.1] font-bold tracking-[-0.02em] text-brand-slate">
            You&apos;re in — let&apos;s get {entry.name} set up.
          </h1>
            ...
            {embedIsPlaceholder ? (
              <div className="flex min-h-[320px] flex-col items-center justify-center gap-3 rounded-[20px] border-2 border-dashed border-border-subtle bg-surface p-10 text-center">
                ...
                <p className="max-w-sm text-sm text-text-muted">
                  {`This is where the ${entry.name} onboarding form will appear once it's built. In the meantime, reach out and we'll get you set up directly.`}
                </p>
              </div>
            ) : (
              <iframe
                src={entry.onboardingFormEmbed}
                title={`${entry.name} onboarding form`}
